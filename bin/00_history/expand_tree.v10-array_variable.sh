...
$(
put_features
echo "cat > ${exeta_home}/rep/${server_type}/${task_type:0:3}/$( instance_file_name ).${sfx} <<EOF"
...
expand_tree () {
    ...
    do
		...
		sql "select 'INSTANCE [' || id::text || '] : ' || name[1]::text || ' ' || array_to_string(name[2:array_length(name, 1)], ' ') from ${schema}.instances where id = ${instance_id}"
                task_features
                call_features
                task_type=$( task_type )
                if   [ "${task_type}" != "call" ]
