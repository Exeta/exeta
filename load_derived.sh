...
instance_id_style="\\033[${no_style}m"
instance_name_style="\\033[${bold}m"
instance_identifier_style="\\033[${no_style}m"
normal_style="\\033[${no_style}m"
...
features () {
    ...
    do
        ...
        then
            ...
            assignment="$(echo "$(substitute "declare -p ${name}")" | sed -e "s/'/''/g")"
            #echo "${name}=\"${value}\""
            sql "update ${schema}.d_feature set value='${value}', assignment='${assignment}' where instance_id = ${instance_id} and name = '${name}'"
            sql "insert into ${schema}.d_feature (instance_id, name, value, assignment) select ${instance_id}, '${name}', '${value}', '${assignment}' where not exists (select null from ${schema}.d_feature where instance_id = ${instance_id} and name = '${name}')"
        fi
    done
}
...
log_instance () {
    log=$(sql "select '${instance_id_style}INSTANCE '||i.id||': ${instance_name_style}'||t.name||'${normal_style} ${instance_identifier_style}'||array_to_string(i.identifier, '${normal_style} ${instance_identifier_style}')||'${normal_style}' from ${schema}.d_instance as i inner join ${schema}.i_task as t on t.id = i.task_id where i.id = ${instance_id}")
    echo -e ${log}
}
...
psql postgres --no-align --tuples-only --field-separator=$'\x07' --record-separator=$'\x08' --command="select * from exeta.i_feature" \
| sed -e 's/\\/\\\\/g' \
| while IFS=$'\x07' read -d $'\x08' task_id call_id position name value ; do echo $task_id $call_id $name $value ; done
